...
public class TestWorkflowMonitor {
  protected static final Logger LOG = LoggerFactory.getLogger(TestWorkflowMonitor.class);
  ...
  private void registerMbean(Object bean, ObjectName name) {
    ...
    try {
      LOG.debug("Register MBean: " + name);
      beanServer.registerMBean(bean, name);
    } catch (Exception e) {
      LOG.debug("Could not register MBean: " + name + e.toString());
    }
  }
